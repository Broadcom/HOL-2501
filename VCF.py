...
#####################################################
# 2501 - Start Auto and IDM - START
#####################################################

if 'vcfauto' in lsf.config['VCF'].keys():
    lsf.write_output('Starting Aria Automation and VMware Identity Manager...')
    lsf.write_vpodprogress('VCF Auto and IDM start up', 'GOOD-3', color=color)
    vcfauto = lsf.config.get('VCF','vcfauto').split('\n')
    lsf.start_nested(vcfauto)

#####################################################
# 2501 - Start Auto and IDM - END
#####################################################

# if not lsf.labcheck:
#     lsf.write_output("Pausing 8 minutes for Auto to start...")
#     lsf.labstartup_sleep(480)

#####################################################
# 2501 - Add Cron Job for Auto - START
#####################################################

remoteFqdn = 'auto.vcf.sddc.lab'
user = 'root'

commands = [
    "rm /var/spool/cron/root",
    "(crontab -l; echo 'SHELL=/bin/sh') | crontab -",
    "(crontab -l; echo 'PATH=/usr/local/sbin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/vmware/bin') | crontab -",
    "(crontab -l; echo '@reboot sleep 900 && /opt/scripts/deploy.sh') | crontab -",
    "sudo shutdown -r now"
]

if not lsf.labcheck:
    if hol.isSshReady(remoteFqdn, user, lsf.password):
        for cmd in commands:
            lsf.write_output(f"2501: running {cmd} on [{remoteFqdn}]")
            print(f"2501: running {cmd} on [{remoteFqdn}]")
            lsf.ssh(cmd, f'{user}@{remoteFqdn}', lsf.password, logfile=lsf.logfile)
            lsf.labstartup_sleep(10)

#####################################################
# 2501 - Add Cron Job for Auto - END
#####################################################
