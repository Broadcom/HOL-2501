...
idmRequestId = None
autoRequestId = None
lcmUsername = "vcfadmin@local"
lcmPassword = "{REPLACE_WITH_PASSWORD}"
###################################
...
# if not lsf.labcheck:
#     if not hol.isAutoReady('auto.vcf.sddc.lab', sslVerify):
#         pid = hol.ssh_async('/opt/scripts/deploy.sh', 'root','auto.vcf.sddc.lab', lsf.password)
#         lsf.write_output(f'2501: Aria Automation Ready Check - DEPLOY.SH [PID={pid}]', logfile=lsf.logfile)
#         print(f'2501: Aria Automation Ready Check - DEPLOY.SH [PID={pid}]')

#         if (pid):
#             print(pid)
#             lsf.ssh([f'echo {pid} > /tmp/pid.txt'], f'root@auto.vcf.sddc.lab', lsf.password, logfile=lsf.logfile)
...
token = hol.getEncodedToken(lcmUsername,lcmPassword)

########################################################
# 2501 - Check Aria Suite Lifecycle is Ready
########################################################

lsf.write_output(f'2501: VMware Workspace ONE Access - Ready Check: {lcmFqdn}', logfile=lsf.logfile)
print(f'2501: VMware Workspace ONE Access - Ready Check: {lcmFqdn}')
while True:
    if hol.isLifecycleReady(lcmFqdn, token, sslVerify):
        print(f'2501: VMware Workspace ONE Access - Ready Check: READY')
        lsf.write_output(f'2501: VMware Workspace ONE Access - Ready Check: READY', logfile=lsf.logfile)
        break
    else:
        print(f'2501: VMware Workspace ONE Access - Ready Check: WAITING')
        lsf.write_output(f'2501: VMware Workspace ONE Access - Ready Check: WAITING', logfile=lsf.logfile)
        time.sleep(15)

########################################################
# 2501 - Trigger Power on for IDM
########################################################

if not hol.isIdmReady(idmFqdn, sslVerify):

    lsf.write_vpodprogress('2501: Workspace ONE Access Power-On - Initiated', 'GOOD-8', color=color)
    print(f'2501: Workspace ONE Access Power-On - Initiated')
    ...
    try:
        ...
        idmRequestId = hol.triggerPowerEvent(lcmFqdn, token, sslVerify, environments, productIds, powerState)
...
########################################################
# 2501 - Trigger Power on for AUTO
########################################################

# if not lsf.labcheck:

#     if not hol.isAutoReady('auto.vcf.sddc.lab', sslVerify):
#         lsf.write_vpodprogress('2501 - Aria Automation Power-On - Initiated', 'GOOD-8', color=color)

#         try:
#             environments = ['Production']
#             productIds = ['vra']

#             autoRequestId = hol.triggerPowerEvent(lcmFqdn, token, sslVerify, environments, productIds, powerState)
